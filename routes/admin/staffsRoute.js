...
// Get All staffs
staffsRoute.get('/', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
...
// Add Staff or Update
staffsRoute.post('/', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
...
staffsRoute.delete('/', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const { id } = req.query;
