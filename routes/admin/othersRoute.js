...
// Shipping
othersRoute.get('/shipping', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const resp = await prisma.shipping_cost.findMany()
    return res.status(200).send(resp)
}))

othersRoute.post('/shipping', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
...
othersRoute.delete('/shipping', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const { id } = req.query
...
othersRoute.get('/term-and-condition', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const resp = await prisma.termCondition.findMany()
    return res.status(200).send(resp[0] || { content: "" })
}))

othersRoute.post('/term-and-condition', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
...
othersRoute.get('/privacy', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const resp = await prisma.privacyPolicy.findMany()
    return res.status(200).send(resp[0] || { content: "" })
}))

othersRoute.post('/privacy', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const { id, content } = req.body;
