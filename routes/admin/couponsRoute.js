...
// Get Coupon
couponsRoute.get('/get', verifyToken(['admin', 'cashier'], 'admin'), asyncError(async (req, res) => {
    const { id } = req.query;
...
couponsRoute.get('/all', verifyToken(['admin', 'cashier'], 'admin'), asyncError(async (req, res) => {
...
couponsRoute.post('/add', verifyToken(['admin', 'cashier'], 'admin'), asyncError(async (req, res) => {
...
couponsRoute.post('/update', verifyToken(['admin', 'cashier'], 'admin'), asyncError(async (req, res) => {
...
couponsRoute.delete('/delete', verifyToken(['admin', 'cashier'], 'admin'), asyncError(async (req, res) => {
    const { id } = req.query;
...
couponsRoute.post('/change-active', verifyToken(['admin', 'cashier'], 'admin'), asyncError(async (req, res) => {
    const { id } = req.body;
    if (!id) throw new apiBadRequestError();
