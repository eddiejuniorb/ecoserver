...
shopRoute.get('/', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const resp = await prisma.our_shop.findMany({ orderBy: { iat: 'desc' } })
    return res.status(200).send(resp)
}))


shopRoute.post('/', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
...
shopRoute.delete('/', verifyToken(['admin'], 'admin'), asyncError(async (req, res) => {
    const { id } = req.query
